...
# Fungsi untuk mengonversi format tanggal dan waktu Edge ke objek datetime Python
def get_edge_datetime(edgedate):
    return datetime(1601, 1, 1) + timedelta(microseconds=edgedate)

# Fungsi untuk mendapatkan kunci enkripsi dari file "Local State" dalam instalasi Edge
...
# Fungsi untuk mendekripsi kata sandi yang dienkripsi dalam basis data Edge
...
# Fungsi utama untuk mengekstrak dan menampilkan informasi login dari basis data Edge
def main():
    # Dapatkan kunci AES
    key = get_encryption_key()
    # Path basis data lokal Edge SQLite
    db_path = os.path.join(os.environ["USERPROFILE"], "AppData", "Local",
                           "Microsoft", "Edge", "User Data", "Default", "Login Data")
    # Salin file ke lokasi lain karena basis data akan terkunci jika Edge sedang berjalan
    ...
    for row in cursor.fetchall():
        ...
        if date_created != 86400000000 and date_created:
            print(f"Tanggal Pembuatan: {str(get_edge_datetime(date_created))}")
        if date_last_used != 86400000000 and date_last_used:
            print(f"Terakhir Digunakan: {str(get_edge_datetime(date_last_used))}")
        print("=" * 50)
    cursor.close()
    db.close()
    try:
